...
- `goalStreak.ts` — streak/longest/total for `weekly` and `count_per_period` goals, counted in *days*: every successful day inside an unbroken chain of periods that met their target (a completed missed period resets it; the still-open current period never breaks it). Used by the goal card/detail, milestone awarding and the Meilensteine page so all three agree; daily goals use the plain functions in `streak.ts`.
